...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        # expand the information maxtrix/vector by one new position
        ...
        # now factor out the precious position
        newlist = range(2,dim+2)
        a = Omega.take([0,1],newlist)
        b = Omega.take([0,1],[0,1])
        c = Xi.take([0,1],[0])
        Omega = Omega.take(newlist,newlist)
        Xi = Xi.take(newlist, [0])
