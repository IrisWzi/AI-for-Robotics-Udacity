...
def plan(warehouse, dropzone, todo):
    ...
    for goal in todo:
        ...
        Astar = [[99 for i in range(len(warehouse[0]))] for j in range(len(warehouse))]
        ...
        while change:
            ...
            for x in range(len(warehouse)):
                for y in range(len(warehouse[0])):
                    if x == gx and y == gy and Astar[x][y] == 99:
                        Astar[x][y] = 0
                        change = True
                    if (x == x0 and y == y0) or warehouse[x][y] == 0:
                        for dir in range(len(direction)):
                            x1 = x + direction[dir][0]
                            y1 = y + direction[dir][1]
                            if x1 >=0 and x1 < len(warehouse) and y1 >= 0 and y1 < len(warehouse[0]):
                                tmp = Astar[x1][y1] + costs[dir]
                                if tmp < Astar[x][y]:
                                    Astar[x][y] = tmp
                                    change = True
